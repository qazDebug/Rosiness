...
namespace Rosiness.Patch
{
    ...
    public class PatchDownloader : IEnumerator
	{
		...
		public void Download()
		{
			...
			RosinessLog.Log($"Begine to download : {TotalDownloadCount} files and {TotalDownloadBytes} bytes");
		...
		public void Update()
		{
			...
			for (int i = _loaders.Count - 1; i >= 0; i--)
			{
				...
				{
					RosinessLog.Error($"Check download content integrity is failed : {element.BundleName}");
					loader.Dispose();
					_loaders.RemoveAt(i);
		...
		private WebFileRequest CreateDownloader(PatchElement element)
		{
			...
			// 创建下载器
			RosinessLog.Log($"Beginning to download web file : {url}");
