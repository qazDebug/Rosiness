/****************************************************
	文件：PatchManagerImpl.cs
	作者：世界和平
	日期：2021/3/19 18:58:15
	功能：Nothing
*****************************************************/
using Rosiness.AI;
...
#pragma warning restore 0649
		...
		{
			RosinessLog.Log($"Beginning to initialize patch manager.");
			...
			// 注意：在覆盖安装的时候，会保留沙盒目录里的文件，所以需要强制清空
			{
				...
				else
				{
					...
					{
						RosinessLog.Warning($"Cache is dirty ! Cache version is {_cache.CacheVersion}, APP version is {Application.version}");
						ClearCache();
			...
			// 加载APP内的补丁清单
			RosinessLog.Log($"Load app patch manifest.");
			{
			...
			// 加载沙盒内的补丁清单
			RosinessLog.Log($"Load sandbox patch manifest.");
		...
		{
			RosinessLog.Log("Begin to run patch procedure.");
			...
			if (msg is PatchEventMessageDefine.OperationEvent)
			{
				...
				if (message.operation == EPatchOperation.BeginingDownloadWebFiles)
				{
					...
					if (_procedure.Current == EPatchStates.GetDonwloadList.ToString())
						...
						RosinessLog.Error($"Patch states is incorrect : {_procedure.Current}");
				}
		...
		{
			RosinessLog.Log("Create internal patch downloader.");
		...
		public bool CheckContentIntegrity(string bundleName)
		{
			...
			{
				RosinessLog.Warning($"Not found check content file in local patch manifest : {bundleName}");
				return false;
			}
		}
		...
		public void CacheDownloadPatchFile(string bundleName)
		{
			if (_localPatchManifest.Elements.TryGetValue(bundleName, out PatchElement element))
			{
				RosinessLog.Log($"Cache download file : {element.BundleName} : {element.Version}");
				_cache.CacheDownloadPatchFile(element.MD5);
			}
			else
			{
				RosinessLog.Warning($"Not found cache content file in local patch manifest : {bundleName}");
			}
		}
		...
		public void SaveRemotePatchManifest(string content)
		{
			...
			// 注意：这里会覆盖掉沙盒内的补丁清单文件
			RosinessLog.Log("Save remote patch manifest.");
