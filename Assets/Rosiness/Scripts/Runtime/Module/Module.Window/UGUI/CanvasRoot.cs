/****************************************************
	文件：CanvasRoot.cs
	作者：世界和平
	日期：2021/1/23 11:20:4
	功能：Nothing
*****************************************************/
using Rosiness.Window;
using UnityEngine;
...
public class CanvasRoot : UIRoot
{
	...
	protected override void OnAssetLoad(GameObject go)
	{
		...
		if (desktopTrans != null)
			...
			RosinessLog.Error("Not found UIDesktop gameObject in UIRoot");

		var cameraTrans = Go.transform.BFSearch("UICamera");
		if (cameraTrans != null)
			UICamera = cameraTrans.GetComponent<Camera>();
		else
			RosinessLog.Error("Not found UICamera gameObject in UIRoot");
	}
